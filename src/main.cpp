...
// #define VK_MESSAGE_SEVERITY VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT
#define VK_MESSAGE_SEVERITY VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT
#define VK_MESSAGE_TYPE VK_DEBUG_UTILS_MESSAGE_TYPE_GENERAL_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_TYPE_VALIDATION_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_TYPE_PERFORMANCE_BIT_EXT

#define VK_CULL_MODE VK_CULL_MODE_BACK_BIT
#define VK_FRONT_FACE VK_FRONT_FACE_CLOCKWISE

#define WIDTH 800
#define HEIGHT 640
#define TITLE "LearnVulkan"
#define ENGINE_NAME "LearnVulkan Engine"
#define EXTRA_IMAGE_COUNT 3
...
{
#ifdef DEBUG
    std::cout << "[INFO]: Reading the file: " << filename << "\n";
#endif
...
#pragma region DebugUtilsMessenger
...
#pragma region SwapChainSupportDetails
...
VkExtent2D chooseSwapExtent(const VkSurfaceCapabilitiesKHR &capabilities, GLFWwindow *window)
{
    ...
    else
    {
        ...
        VkExtent2D actualExtent = {static_cast<unsigned int>(width), static_cast<unsigned int>(height)};
...
#pragma region createSyncObjects()
    void createSyncObjects()
    {
        ...
        for (unsigned int i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
            if (vkCreateSemaphore(device, &semaphoreInfo, nullptr, &imageAvailableSemaphores[i]) != VK_SUCCESS || vkCreateSemaphore(device, &semaphoreInfo, nullptr, &renderFinishedSemaphores[0]) != VK_SUCCESS || vkCreateFence(device, &fenceInfo, nullptr, &inFlightFences[i]) != VK_SUCCESS)
                throw std::runtime_error("[ERROR]: Failed to create semaphores or a fences!");
    }
#pragma endregion
...
#pragma region createCommandBuffers()
    void createCommandBuffers()
    {
        ...
        if (vkAllocateCommandBuffers(device, &allocInfo, commandBuffers.data()) != VK_SUCCESS)
            throw std::runtime_error("[ERROR]: Failed to allocate a command buffers!");
    }
#pragma endregion
...
#pragma region createFramebuffers()
...
#pragma region createGraphicsPipeline()
    void createGraphicsPipeline()
    {
        ...
        rasterizer.frontFace = VK_FRONT_FACE;
        ...
        pipelineInfo.stageCount = static_cast<unsigned int>(shaderStages.size());
...
#pragma region createShaderModule()
    ...
    void createImageViews()
    {
        ...
        for (unsigned int i = 0; i < swapChainImages.size(); i++)
        {
            ...
            if (vkCreateImageView(device, &createInfo, nullptr, &swapChainImageViews[i]) != VK_SUCCESS)
                throw std::runtime_error("[ERROR]: Failed to create swap chain image view!");
        }
    }
#pragma endregion
...
#pragma region createSwapChain()
    void createSwapChain()
    {
        ...
        unsigned int imageCount = swapChainSupport.capabilities.minImageCount + EXTRA_IMAGE_COUNT;
...
#pragma region recordCommandBuffer()
...
#pragma region isDeviceSuitable()
...
#pragma region checkDeviceExtensionSupport()
...
#pragma region populateDebugMessengerCreateInfo()
    void populateDebugMessengerCreateInfo(VkDebugUtilsMessengerCreateInfoEXT &createInfo)
    {
        createInfo = {};

        createInfo.sType = VK_STRUCTURE_TYPE_DEBUG_UTILS_MESSENGER_CREATE_INFO_EXT;
        createInfo.messageSeverity = VK_MESSAGE_SEVERITY;
        createInfo.messageType = VK_MESSAGE_TYPE;
...
#pragma region createInstance()
    void createInstance()
    {
        if (enableValidationLayers && !checkValidationLayerSupport())
            throw std::runtime_error("[ERROR]: Required validation layers are not available!");
...
#pragma region getRequiredExtensions()
    std::vector<const char *> getRequiredExtensions()
    {
...
#ifdef DEBUG
        std::cout << "[INFO]: Required " << requiredExtensions.size() << " number of extensions: \n";
        for (unsigned int i = 0; i < requiredExtensions.size(); i++)
            std::cout << "  Extension number " << i + 1 << ": " << requiredExtensions[i] << "\n";
        ...
        for (unsigned int i = 0; i < extensionCount; i++)
            std::cout << "  Extension number " << i + 1 << ": " << extensions[i].extensionName << "\n";
#endif
...
#pragma region checkValidationLayerSupport()
...
#pragma region debugCallback()
    static VKAPI_ATTR VkBool32 VKAPI_CALL debugCallback(
        ...
        void *pUserData)
    {
        std::cerr << "[VALIDATION_LAYER]: " << pCallbackData->pMessage << "\n";
